...
from src.config import Config, get_config, load_config_from_db, save_config_to_db
...
class BotManager:
    ...
    async def load_bots(self):
        ...
        # Refresh global config from DB if it exists
        db_config = await load_config_from_db(self.database)
        if db_config:
            self._global_config = db_config
            # Save it back to ensure DB is seeded if it was just loaded from file/defaults
            await save_config_to_db(self.database, self._global_config)
        ...
        if saved_configs and "bots" in saved_configs:
            for bot_id, bot_conf_dict in saved_configs["bots"].items():
                logger.info(
                    "restoring_bot",
                    bot_id=bot_id,
                    symbol=bot_conf_dict.get("trading", {}).get("symbol"),
                )
                try:
    ...
    def _create_bot_instance(self, bot_id: str, config_dict: dict) -> OmniTrader:
        ...
        bot_config = Config.model_validate(merged_dict)
    ...
    def get_bot(self, bot_id: str) -> OmniTrader | None:
