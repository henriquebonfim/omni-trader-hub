...
class GraphAnalytics:
    ...
    async def get_asset_sentiment(self, symbol: str, hours_lookback: int = 24) -> Dict[str, Any]:
        """
        ...
        """
        timestamp_threshold = int((datetime.now(timezone.utc).timestamp() - (hours_lookback * 3600)) * 1000)
        ...
        RETURN 
            ...
            sum(CASE WHEN r.sentiment > 0 THEN 1 ELSE 0 END) as positive_mentions,
            sum(CASE WHEN r.sentiment < 0 THEN 1 ELSE 0 END) as negative_mentions
        """
        
        try:
            async with self.db._driver.session() as session:
                result = await session.run(query, symbol=symbol, timestamp_threshold=timestamp_threshold)
                ...
                return {"avg_sentiment": 0.0, "mention_count": 0, "positive_mentions": 0, "negative_mentions": 0}
        except Exception as e:
            logger.error("graph_analytics_sentiment_error", symbol=symbol, error=str(e))
            return {"avg_sentiment": 0.0, "mention_count": 0, "positive_mentions": 0, "negative_mentions": 0}

    async def detect_sector_contagion(self, symbol: str, hours_lookback: int = 24) -> Dict[str, Any]:
        """
        ...
        try:
            async with self.db._driver.session() as session:
                ...
                if record and record["sector_name"]:
                    return {
                        "contagion_risk": True if record["peers_in_distress"] > 2 else False,
                        "sector": record["sector_name"],
                        "peers_in_distress": int(record["peers_in_distress"]),
                        "sector_distress_level": float(record["sector_distress_level"] or 0.0)
                    }
                return {"contagion_risk": False, "sector": None, "peers_in_distress": 0, "sector_distress_level": 0.0}
        except Exception as e:
            logger.error("graph_analytics_contagion_error", symbol=symbol, error=str(e))
            return {"contagion_risk": False, "sector": None, "peers_in_distress": 0, "sector_distress_level": 0.0}

    async def detect_sentiment_divergence(self, symbol: str, current_price: float, hours_lookback: int = 24) -> Dict[str, Any]:
        """
