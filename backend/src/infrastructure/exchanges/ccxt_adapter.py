...
class CCXTExchange(BaseExchange):
    ...
    def _check_paper_orders(
        ...
        """
        Evaluate active paper protective orders (SL/TP) against market price range.

        Args:
            current_price: Current market price (close/last)
            high_price: Maximum price since last check (for range simulation)
            low_price: Minimum price since last check (for range simulation)
        """
        ...
        # Use High/Low for simulation if available, otherwise fallback to current_price
        h = high_price if high_price is not None else current_price
        l = low_price if low_price is not None else current_price

        triggered_order = None
        trigger_price = None

        # Priority: Check SL before TP for conservative simulation
        sl_order = next(
            (o for o in self._paper_orders if o.get("type") == "stop_market"), None
        )
        tp_order = next(
            (o for o in self._paper_orders if o.get("type") == "take_profit_market"),
            None,
        )

        # 1. Check Stop Loss
        if sl_order and sl_order.get("status") == "open":
            stop_price = sl_order.get("stopPrice")
            if stop_price is not None:
                if (side == "long" and l <= stop_price) or (
                    side == "short" and h >= stop_price
                ):
                    triggered_order = sl_order
                    trigger_price = stop_price

        # 2. Check Take Profit (only if SL didn't trigger)
        if not triggered_order and tp_order and tp_order.get("status") == "open":
            tp_price = tp_order.get("take_profit_price") or tp_order.get("stopPrice")
            if tp_price is not None:
                if (side == "long" and h >= tp_price) or (
                    side == "short" and l <= tp_price
                ):
                    triggered_order = tp_order
        ...
        if triggered_order and trigger_price is not None:
            ...
            # Calculate PnL (Standard: (exit - entry) * qty)
            if side == "long":
        ...
        if self.paper_mode:
            ...
            if last_price is not None:
                self._check_paper_orders(
                    ...
                    high_price=float(ticker.get("high")) if ticker.get("high") else None,
                    low_price=float(ticker.get("low")) if ticker.get("low") else None,
                )
