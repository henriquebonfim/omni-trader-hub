        ...
        async def wrapper(self, *args, **kwargs):
            ...
            for attempt in range(retries):
                try:
                    ...
                    last_exception = e
                    logger.warning(
                        "database_operation_failed_retrying",
                    ...
                    if attempt < retries - 1:
                        ...
                        # Reconnect if driver is lost
                        if not self._driver or not await self._is_connected():
                            try:
                                await self.connect()
                            except Exception:
                                pass
            logger.error("database_operation_failed_permanently", operation=func.__name__, error=str(last_exception))
...
class MemgraphDatabase(BaseDatabase):
    ...
    async def _is_connected(self) -> bool:
        """Check if driver is alive and responding."""
        if not self._driver:
            return False
        try:
            async with self._driver.session() as session:
    ...
    async def set_state(
        self, key: str, value: dict, expires_in: int | None = None
    ) -> None:
        """
    ...
    async def get_custom_strategy(self, name: str) -> dict | None:
