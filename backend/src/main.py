...
class OmniTrader:
    ...
    def __init__(self):
        ...
        # Initialize event bus and use cases (DDD)
        self.event_bus = EventBus()
        self.open_position_use_case = OpenPositionUseCase(
    ...
    def _setup_event_handlers(self) -> None:
        ...
        async def on_emergency_close(event) -> None:
            """Handle EmergencyClose domain event."""
            payload = event.payload
            await self.notifier.send(
                f"🚨 **EMERGENCY CLOSE**: {payload['symbol']} closed due to {payload['reason']}"
            )
    ...
    async def _execute_signal(
        ...
        if result.signal == Signal.LONG:
            await self.open_position_use_case.execute(
                symbol=symbol,
                side="long",
                current_price=current_price,
