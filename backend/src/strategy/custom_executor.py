...
class CustomStrategyExecutor(BaseStrategy):
    ...
    def __init__(self, config: Config, custom_config: Dict[str, Any]):
        ...
        if custom_config.get("take_profit_atr_mult"):
            self.config.risk.take_profit_atr_mult = custom_config[
                "take_profit_atr_mult"
            ]
        if custom_config.get("min_bars_between_entries"):
            self.config.strategy.min_bars_between_entries = custom_config[
            ...
            try:
                ...
                if isinstance(result, list) or isinstance(result, tuple):
                    ...
                    if len(result) > 0 and isinstance(
                        ...
                        self._indicators[output_name] = pd.Series(
                            result[0], index=ohlcv.index
                        )
                    else:
                        pass
                else:
                ...
                logger.error(
                    "custom_strategy_indicator_failed",
                    strategy=self.name,
    ...
    def _get_value(self, item: Any, idx: int = -1) -> float:
        ...
        if (
            isinstance(item, str)
            and self.ohlcv is not None
            and item in self.ohlcv.columns
        ):
    ...
    def should_exit(self) -> bool:
        if self.current_position == "long":
            return self._evaluate_conditions(
                self.custom_config.get("exit_long_json", [])
            )
        elif self.current_position == "short":
            return self._evaluate_conditions(
                self.custom_config.get("exit_short_json", [])
            )
        return False
