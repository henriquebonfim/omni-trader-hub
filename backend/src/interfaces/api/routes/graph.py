...
def _to_ms(val) -> int:
    ...
    if isinstance(val, (int, float)):
        # If it's already a timestamp, assume it's in ms if > 1e12, else s
        if val > 1e12:
            return int(val)
        return int(val * 1000)
    try:
        ...
        items.append(
            {
                ...
                "published_at": _to_ms(record.get("published_at")),
                ...
                news.append(
                    {
                        ...
                        "timestamp": _to_ms(record.get("timestamp", 0)),
    ...
    return [
        {
            ...
            "timestamp": _to_ms(r.get("timestamp", 0)),
        }
        for r in records
    ]
...
async def get_correlation_matrix(
    ...
    if len(returns_columns) == 1:
        return {
            ...
            "matrix": [[1.0]],
            "as_of": int(datetime.now(UTC).timestamp() * 1000),
        }
