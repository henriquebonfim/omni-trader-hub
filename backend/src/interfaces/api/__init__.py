...
def create_api(bot_instance=None, bot_manager=None) -> FastAPI:
    ...
    async def auth_middleware(request: Request, call_next):
        ...
        Requires Authorization header with valid Bearer token for all routes except public ones.
        """
        path = request.url.path

        # Allow public paths without authentication
        if path in PUBLIC_PATHS or path == "/api/health":
        ...
        if not hmac.compare_digest(token, get_api_key()):
            return JSONResponse(
                status_code=status.HTTP_401_UNAUTHORIZED,
