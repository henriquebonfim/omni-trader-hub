...
def calculate_metrics(trades: List[Dict[str, Any]], initial_balance: float) -> Dict[str, Any]:
    """
...
def bootstrap_confidence_intervals(trades: List[Dict[str, Any]], iterations: int = 1000, confidence_level: float = 0.95) -> Dict[str, Any]:
    """
    ...
    return {
        "net_profit_ci_lower": float(np.percentile(bootstrap_net_profits, lower_percentile)),
        "net_profit_ci_upper": float(np.percentile(bootstrap_net_profits, upper_percentile))
    }

def generate_walk_forward_splits(candles: List[Dict[str, Any]], train_months: int = 6, test_months: int = 1) -> List[Dict[str, Any]]:
    """
