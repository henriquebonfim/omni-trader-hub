...
class BacktestEngine:
    ...
    def __init__(self, strategy: BaseStrategy, config: Config, initial_balance: float = 10000.0):
    ...
    def _execute_trade(self, side: str, price: float, timestamp: int, size: float, reason: str, pnl: float = 0.0, pnl_pct: float = 0.0):
        trade = {
            "timestamp": timestamp,
            "symbol": self.symbol,
            "side": side,
    ...
    def run(self, candles: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for _i, candle in enumerate(sorted_candles):
            ...
            # Analyze
            current_pos_str = self.current_position["side"] if self.current_position else None
            ...
            if result.signal == Signal.LONG and not self.current_position:
                self._open_position("long", candle["close"], candle["timestamp"], result.reason)
            elif result.signal == Signal.SHORT and not self.current_position:
                self._open_position("short", candle["close"], candle["timestamp"], result.reason)
            elif result.signal in (Signal.EXIT_LONG, Signal.EXIT_SHORT) and self.current_position:
                self._close_position(candle["close"], candle["timestamp"], result.reason)

        # Close any open position at the end of the backtest
        if self.current_position and sorted_candles:
            self._close_position(sorted_candles[-1]["close"], sorted_candles[-1]["timestamp"], "end_of_backtest")
