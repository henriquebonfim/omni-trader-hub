...
def _check_rate_limit(client_ip: str):
    ...
    _rate_limits[client_ip] = [
        ts for ts in _rate_limits[client_ip] if now - ts < RATE_LIMIT_WINDOW_SEC
    ]
...
async def compute_indicator(
    request: Request, body: ComputeRequest, api_key: str = Depends(verify_api_key)
):
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=400,
            detail=f"Unknown or unsupported TA-Lib function: {body.function}",
        ) from None
    ...
    candles = await bot.database.get_candles(
        symbol=body.symbol,
    ...
    if not candles:
        raise HTTPException(
            status_code=400,
            detail=f"No candles found for {body.symbol} {body.timeframe}",
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=400, detail=f"Error computing indicator: {str(e)}"
        ) from e
