...
async def test_graph_analytics_contagion():
    db = MockMemgraph(
        {"sector_name": "DeFi", "peers_in_distress": 3, "sector_distress_level": -0.6}
    )
    ga = GraphAnalytics(db)
...
async def test_evaluate_automated_crisis():
    ...
    # Not bad enough
    assert (
        await cm.evaluate_automated_crisis(contagion_risk=False, sentiment_level=-0.5)
        is False
    )
    assert await cm.is_crisis_active() is False
    ...
    # Bad enough
    assert (
        await cm.evaluate_automated_crisis(contagion_risk=True, sentiment_level=-0.9)
        is True
    )
    assert await cm.is_crisis_active() is True
