...
def test_restart_system_no_open_positions(client, mock_bot, monkeypatch):
    mock_position = MagicMock()
    mock_position.is_open = False
    mock_bot.exchange.fetch_position = AsyncMock(return_value=mock_position)

    # Mock os.kill to prevent actual killing of the test runner
    mock_kill = MagicMock()
    monkeypatch.setattr("os.kill", mock_kill)

    response = client.post("/api/system/restart", json={"force": False}, headers={"Authorization": "Bearer test-secret"})
...
def test_restart_system_with_open_positions_with_force(client, mock_bot, monkeypatch):
    ...
    response = client.post("/api/system/restart", json={"force": True}, headers={"Authorization": "Bearer test-secret"})
    assert response.status_code == 200
    assert response.json()["ok"] is True
