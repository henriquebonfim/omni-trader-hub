...
class MockDatabase:
    ...
    async def save_custom_strategy(self, name, description, regime_affinity, entry_long_json, entry_short_json, exit_long_json, exit_short_json, indicators_json, stop_loss_atr_mult, take_profit_atr_mult, min_bars_between_entries):
        self.strats[name] = {
            "name": name,
...
class MockBot:
    def __init__(self, db):
        self.config = Config({"exchange": {"paper_mode": True}, "trading": {"timeframe": "1h"}, "strategy": {"name": "ema_volume"}})
        self.database = db
        self.strategy = None
